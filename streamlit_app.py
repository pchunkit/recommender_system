...
movies = pd.io.parsers.read_csv('ml-1m/movies.dat',
                                ...
                                engine='python', delimiter='::', encoding='unicode_escape')
...
class content_based_filtering:
    ...
    def calc(self, movie_title):
        indices = list(enumerate(
            self.similarity_matrix[movies[movies['title'] == movie_title].index][0]))
...
class collaborative_based_filtering:
    def __init__(self):
        if os.path.exists("svd_matrix.npy"):
            ...
            ratings_mat = np.ndarray(shape=(np.max(rating.movie_id.values), np.max(
                rating.user_id.values)), dtype=np.uint8)
            ratings_mat[rating.movie_id.values-1,
                        rating.user_id.values-1] = rating.rating.values
            normalised_mat = ratings_mat - \
                np.asarray([(np.mean(ratings_mat, 1))]).T
    ...
    def svd_cosine_similarity(self, data, movie_id, top_n=10):
        ...
        top_indexes = sort_indexes[1:top_n+1]
        top_similarity = [similarity[index] for index in top_indexes]
        recommended_movies = [movies.loc[movies["movie_id"] == (
            i + 1), "title"].values[0] for i in top_indexes]
    ...
    def calc(self, movie_title):
        ...
        movie_id = movies.loc[movies["title"] ==
                              movie_title, "movie_id"].values[0]
        svd_recommended_movies = self.svd_cosine_similarity(
            sliced, movie_id, top_n)
...
if __name__ == "__main__":
    ...
    st.write("Genre of {}: {}".format(movie_option,
             movies.loc[movies['title'] == movie_option, 'genre'].item()))
#     recommended_ls = []

#     st.write("Collaborative-based Filtering")
    recommended_movies, top_index, similarity_score = collab_recommender.calc(
        movie_option)
...
#     recommended_ls = list(set(recommended_ls))
    ...
    for i in range(int(len(recommended_movies)/ncol)):
        ...
        for idx, (m_name, m_id) in enumerate(zip(recommended_movies[i*ncol: (i*ncol)+4], top_index[i*ncol: (i*ncol)+4])):
            genre = movies.loc[movies['movie_id'] ==
                               m_id, 'genre'].values[0].split("|")
